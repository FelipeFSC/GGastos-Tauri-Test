  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  rotuloPeriodo(): string {
    ...
    switch (this.periodoTipo) {
      ...
      case "semana":
        return (
          this.formatarDiaMes(inicio) + " a " + this.formatarDiaMes(fim)
        );
  ...
  private montarLinhas(lancamentos: LancamentoRow[]): LinhaCategoria[] {
    ...
    for (const [paiId, grupo] of grupos) {
      ...
      const nome = grupo.categoriaPai ? grupo.categoriaPai.nome : "Sem categoria";
      ...
      if (doSubcategorias.length === 0) {
        ...
        for (const [chave, itens] of porSubcategoria) {
          ...
          const rotulo =
            chave === paiId ? `${nome} (categoria pai)` : subCategoria?.nome ?? "-";
