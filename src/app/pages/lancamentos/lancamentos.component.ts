...
export class LancamentosComponent implements OnInit {
  ...
  async carregarLancamentos(): Promise<void> {
    ...
    const proximoMesNumero = String(
      proximoMes.getMonth() + 1
    ).padStart(2, "0");

    const primeiroDiaProximoMes =
      `${proximoAno}-${proximoMesNumero}-01`;
  ...
  organizarPorDia(): void {
    ...
    this.gruposLancamentos = Array.from(
      grupos.entries()
    ).map(([data, lancamentos]) => ({
      data,
  ...
  async editarLancamento(
    ...
    this.form.valor =
      this.valorFormatado(lancamento.valor);
    ...
    this.form.confirmado =
      lancamento.confirmado === 1;
    ...
    if (this.editandoGrupo) {
      this.tipoRepeticaoOriginal =
        lancamento.fixo === 1 ? "fixa" : "parcelada";
  ...
  alternarOpcao(
    opcao: "repetir" | "observacao" | "anexo" | "tags"
  ): void {
    /*
  ...
  nomeCategoria(
    ...
    return (
      this.categoria(lancamento)?.nome ??
      "Sem categoria"
    );
  }
  ...
  iconeCategoria(
    ...
    return (
      this.categoria(lancamento)?.icone ||
      "category"
    );
  }
  ...
  private adicionarIntervalo(
    ...
    novaData.setMonth(
      novaData.getMonth() + mesesAdicionar
    );
  ...
  async salvarLancamento(): Promise<void> {
    ...
    try {
      ...
      if (this.editarId) {
        await this.db.atualizarLancamentoComEscopo(
          this.editarId,
          {
            ...
            parcela_total:
              this.editandoGrupo &&
              this.form.tipoRepeticao === "parcelada"
        ...
        if (escopoUsado === "atual") {
          ...
          await this.db.salvarTagsDoLancamento(
            this.editarId,
            this.form.tagIds
          );
       ...
       */
        ...
        await this.salvarLancamentoFixo(
          descricao,
          valor,
          origem
        );
  ...
  private async inserirLancamento(
    ...
    if (lancamento.cartao_id) {
      const fatura =
        await this.db.obterOuCriarFatura(
          lancamento.cartao_id,
          lancamento.data
        );
  ...
  private async salvarLancamentoFixo(
    ...
    if (
      ...
      throw new Error(
        "Frequência de repetição inválida."
      );
    }
  ...
  private async inserirLancamentoComRepeticao(
    ...
    await this.db.run(
      ...
      [
        ...
        repeticao.primeiraOcorrencia ? this.form.observacao.trim() || null : null,
      ]
    );
